...
export const validateMediaForPlatform = ({
  ...
  for (const platformEntry of platformData) {
    if (
      "fileIds" in platformEntry &&
      platformEntry.fileIds &&
      platformEntry.fileIds.length > 0
    ) {
      ...
      const rule = mediaRules[type as keyof typeof mediaRules] as
        | ZodTypeAny
        | undefined;
      if (!rule) {
        throw new Error(
          `No media rules defined for type: ${type} on platform: ${platform}`,
        );
      }

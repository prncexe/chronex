...
export const linkedinOAuthProcedure = workspaceProcedure.mutation(async ({ ctx }) => {
  const userId = ctx.user.id
  const token = await ctx.db.query.authToken.findFirst({
    where: (account, { eq, and }) =>
      and(eq(account.userId, userId), eq(account.platform, 'linkedin')),
  })
  if (!token) {
    throw new TRPCError({
      code: 'NOT_FOUND',
      message: 'linkedin token not found',
    })
  }

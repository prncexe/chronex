import z from "zod";
import {instagram, linkedin, threads,slack,discord} from "@/lib/media-validation/platform-rules/combined";
...
export const InputSchema =  z.object({
  ...
  .refine(arr => new Set(arr).size === arr.length, {
    message: "Duplicate platforms are not allowed",
      ...
      scheduledAt: z.date(),
      platformdata: PlatformDataArraySchema.min(1, "Provide at least one platform data entry").superRefine((arr, ctx) => {

...
async function publishContainer(token: AuthToken, creationId: string): Promise<PublishResponse> {
...
export const InstagramImage = async (payload: PlatformJobPayload, env: Env): Promise<void> => {
...
export const InstagramReel = async (payload: PlatformJobPayload, env: Env): Promise<void> => {
  const db = (await import('@repo/db')).createDb(env.DATABASE_URL)
...
export const InstagramCarousel = async (payload: PlatformJobPayload, env: Env): Promise<void> => {
  const db = (await import('@repo/db')).createDb(env.DATABASE_URL)
  ...
  try {
    ...
    if (payload.phase === 'check_status' && payload.childContainerIds?.length) {
      ...
      await enqueueStatusCheck(env, payload, carousel.id, payload.childContainerIds)
      return
    }
...
export const InstagramStory = async (payload: PlatformJobPayload, env: Env): Promise<void> => {
  const db = (await import('@repo/db')).createDb(env.DATABASE_URL)
  ...
  try {
    ...
    if (!probe.ok) {
      throw new Error(`Media URL not accessible (HTTP ${probe.status}): ${media.url}`)
    }

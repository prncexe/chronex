...
async function postMessage(token: AuthToken, channelId: string, text: string): Promise<string> {
...
async function uploadFile(
  ...
  if (!urlData.ok || !urlData.upload_url || !urlData.file_id) {
    throw new Error(`Slack files.getUploadURLExternal failed: ${urlData.error ?? 'Unknown error'}`)
  }
...
export const SlackMessage = async (payload: PlatformJobPayload, env: Env): Promise<void> => {
...
export const SlackFile = async (payload: PlatformJobPayload, env: Env): Promise<void> => {
  const db = (await import('@repo/db')).createDb(env.DATABASE_URL)
  const data = payload.metadata as SlackMetadata
  ...
  try {
    ...
    for (let i = 0; i < mediaItems.length; i++) {
      ...
      const fileName = urlPath.split('/').pop() || `file_${i}.${item.extension ?? 'bin'}`
